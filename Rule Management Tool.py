...
def save_config(data):
    with open(CONFIG_PATH, "w", encoding="utf-8") as f: # 파일 열기
...
class ProductManagerUI(QWidget):
    ...
    def init_ui(self):
        ...
        src_row = QHBoxLayout()
        src_row.addWidget(self.in_src)
        src_row.addWidget(self.btn_src)
        fv.addRow("Source", src_row)

        # Destination
        self.in_dst = QLineEdit()
        self.btn_dst = QPushButton("Browse")
        self.btn_dst.clicked.connect(self.browse_destination)

        dst_row = QHBoxLayout()
        dst_row.addWidget(self.in_dst)
        dst_row.addWidget(self.btn_dst)
        fv.addRow("Destination", dst_row)
    ...
    def browse_destination(self):
        f, _ = QFileDialog.getOpenFileName(self, "Select Destination", "", "All Files (*)")
        if f:
            self.in_dst.setText(f)
    
    def browse_source(self):
        f, _ = QFileDialog.getOpenFileName(self, "Select Source File", "", "All Files (*)")
        if f:
            self.in_src.setText(f)
    ...
    def delete_txt_rule(self):
        ...
        product = p_item.text() # 현재 선택된 제품 이름
        version = v_item.text() # 현재 선택된 버전 이름
        ...
        rules_list = self.config["products"][product]["versions"][version]["rules"] # ruels 리스트 가져오기
    ...
    def delete_version(self):
        p_item = self.product_list.currentItem() # 현재 선택된 제품 항목 가져오기
        v_item = self.version_list.currentItem() # 현재 선택된 버전 항목 가져오기

        if not p_item or not v_item:
            QMessageBox.warning(self, "Error", "삭제할 버전을 선택해 주십시오.")
            return
